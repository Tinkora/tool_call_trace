...
- [ ] Add one minimal timestamped tool-call fixture derived from a pinned
  official OpenAI Agents, LangChain, and PydanticAI test/export contract.
- [ ] Add failing tests that call the not-yet-implemented format parsers and
  assert stable IDs, names, timing, input, output, and error status.
- [ ] Run `cargo test -p tool_call_trace_core --test import_contracts --locked`
  and confirm the tests fail because the parser APIs do not exist.
- [ ] Commit the fixtures and red tests as `test: add pinned Agent SDK trace contracts`.
...
- [ ] Add recognized-shape probes so malformed recognized inputs fail without
  format fallthrough.
- [ ] Parse RFC 3339 timestamps with checked millisecond conversion and flatten
  only real tool/function spans.
- [ ] Reuse common normalization, duplicate-ID, ordering, duration, and resource
  validation helpers.
- [ ] Add the 100,000-line boundary to every raw input parser.
- [ ] Run focused importer tests, then workspace tests and Clippy.
- [ ] Commit as `feat: import timestamped Agent SDK traces`.
...
- [ ] Add failing tests for authorization fields, API-key-like keys, URL
  user-info/query/fragment removal, configured JSON Pointer paths, arrays,
  errors, and unchanged call/trace IDs.
- [ ] Add failing tests proving secret sentinels are absent from serialized
  outcomes and error messages.
- [ ] Implement the smallest recursive transformer with exact configured paths,
  a documented key allowlist, HTTP(S)-only URL parsing, and `[REDACTED]`.
- [ ] Expose JSON-compatible WASM redaction and run focused, workspace, Clippy,
  and WASM checks.
- [ ] Commit as `feat: add opt-in local trace redaction`.
...
- [ ] Add failing process tests for stdin/file input, explicit/auto formats,
  redacted JSON, configured paths, invalid input, and non-zero failures.
- [ ] Implement argument parsing without a command framework dependency and
  keep normalized JSON on stdout and diagnostics on stderr.
- [ ] Run CLI tests, workspace tests, Clippy, and a manual stdin smoke check.
- [ ] Commit as `feat: add the local trace contract CLI`.
...
- [ ] Add failing browser tests for auto-detection, the off-by-default checkbox,
  path validation, announced redaction count, secret-free details/tooltips, and
  preserved IDs.
- [ ] Replace the two-option selector with a labeled format menu, add the
  redaction controls, and route all displayed/analyzed data through Rust/WASM.
- [ ] Run the compiled WASM smoke suite and Chromium checks at 375, 768, 1024,
  and 1440 pixels, including console, traffic, keyboard, reduced motion, and
  overflow assertions.
- [ ] Commit as `feat: expose local trace redaction in the browser`.
...
- [ ] Document exact formats, limits, redaction rules, CLI usage, limitations,
  and fixture provenance in English-first bilingual docs.
- [ ] Remove the obsolete non-goal that says redaction is unavailable.
- [ ] Run repository documentation checks and the complete Rust/WASM/browser
  gate from `AGENTS.md`.
- [ ] Review staged changes for secret sentinels and unintentional generated

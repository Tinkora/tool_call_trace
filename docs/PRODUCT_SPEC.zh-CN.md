...
5. 在瀑布流中比较调用位置和耗时。
6. 查看脱敏后的输入、输出、状态和错误。
7. 检查聚合延迟、错误率、重复调用、慢调用和重试循环。

## 重试分析契约

- 重试循环至少包含三次按时间顺序执行的相同失败调用；后一次调用必须在前一次结束时
  或之后开始。
- 该序列之后紧接的相同成功调用会将 finding 变为 `recovered_retry_loop`。
- 时间范围重叠的调用会聚合成一个 `overlapping_duplicate` 组，而不会归类为重试。
- 调用身份使用去除首尾空白并统一大小写的工具名与 canonical JSON 输入。
- finding 的 `call_count` 保留完整数量，但最多包含 20 个调用 ID；省略 ID 时设置
  `call_ids_truncated`。
- 身份分析在可选脱敏之前针对解析后的源数据执行。finding 不复制 input 或 error 值；
  工具名和调用 ID 仍是用户控制的标识符，因此不声称它们一定不含秘密。
